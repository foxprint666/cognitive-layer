    ...
    def apply(self, engine: Any, ne: float, ach: float) -> None:
        ...
        if engine.workspace is not None:
            ...
            if selector is not None and hasattr(selector, "ignition_threshold"):
                if self.baseline_ignition_threshold is None:
                    self.baseline_ignition_threshold = float(
                        selector.ignition_threshold
                    )

                # Formula: ignition_threshold = baseline - (beta * NE) + (gamma * ACh)
                new_thresh = (
                    self.baseline_ignition_threshold
                    - (self.beta * ne)
                    + (self.gamma * ach)
                )
                selector.ignition_threshold = max(0.0, min(1.0, float(new_thresh)))
                logger.debug(
                    f"Modulated GWT ignition threshold to {selector.ignition_threshold:.4f}"
                )
        ...
        for adapter in engine.registry.list_adapters():
            ...
            if gate is not None:
                ...
                gate.threshold = max(
                    0.0, min(1.0, float(base_thresh + self.threshold_coef * ach))
                )

                # Formula 2: Gain Sigmoid Temperature drops with ACh focus
                # temp_t = 1.0 - 0.5 * ACh_t
                gate.gain_temperature = max(
                    1e-5, min(1.0, float(1.0 - self.temp_coef * ach))
                )
                logger.debug(
    ...
    def modulate(self, engine: Any) -> None:
        ...
        with torch.no_grad():
            ...
            if workspace is not None:
                ...
                if (
                    last_weights is not None
                    and last_query is not None
                    and last_keys is not None
                ):
                    ...
                    workspace_key = (last_keys * last_weights.unsqueeze(-1)).sum(
                        dim=1
                    )  # [B, key_dim]
                    cos_sim = F.cosine_similarity(
                        workspace_key, last_query, dim=-1
                    )  # [B]
