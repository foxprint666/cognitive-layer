...
def test_cognitive_output_router():
    output_specs = {"classification": 3, "actions": 5, "salience_feedback": 2}
    ...
    try:
        assert (
            outputs["classification"].untyped_storage().data_ptr()
            == outputs["actions"].untyped_storage().data_ptr()
        )
    except AttributeError:
        assert (
            outputs["classification"].storage().data_ptr()
            == outputs["actions"].storage().data_ptr()
        )
