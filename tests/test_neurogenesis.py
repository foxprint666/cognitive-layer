...
def test_neurogenesis_astrocyte_regulation():
    ...
    assert torch.mean(torch.abs(regulated)).item() < torch.mean(torch.abs(x_hyper)).item()
...
def test_dynamic_optimizer_parameter_update():
    ...
    assert not any(group.get("name") == f"dynamic_branch_{new_idx}" for group in optimizer.param_groups)
...
def test_neurogenesis_consolidation_offline_sleep():
    ...
    new_idx = model.adapter.add_dendritic_branch()
    assert len(model.adapter.branches) == 2
    assert model.adapter.branch_states["state_1"][2].item() == 0.0  # consolidation score initially 0.0
    ...
    consolidation_score = state_after[2].item()
