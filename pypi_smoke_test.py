"""
Smoke test: verifies cognitive-aug 0.4.0 installs and imports correctly from PyPI.
Run this inside a fresh venv AFTER: pip install cognitive-aug==0.4.0
"""
...
assert version == "0.4.0", f"Expected 0.4.0, got {version}"
...
w1 = cos_sel(keys, query)
assert w1.shape == (B, 3), f"CosineSel shape: {w1.shape}"
w2 = vec_sel(keys, query)
assert w2.shape == (B, 3), f"VecCrossAttn shape: {w2.shape}"
w3 = gum_sel(keys, query)
assert w3.shape == (B, 3), f"GumbelSel shape: {w3.shape}"
print("  ✓ Selectors OK")
...
s3 = temp_sal(states)
assert s3.shape == (B, 2)
gate = temp_sal.gate(s3)
assert gate.shape == (B, 2)
...
print("\n========================================")
print("  ALL TESTS PASSED — cognitive-aug 0.4.0")
print("========================================")
